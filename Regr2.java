...
public class Regr2 extends Frame{
	...
	Regr2(final int x,final int y){
		...
		((Button)pbot.add(new Button("Calculate"))).addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				...
				c.add(new Label("Linear Regression equation is y = a1 x + a0",Label.LEFT));
				c.add(new Label("So the linear regression equation is: y = "+ CalculateAone(fieldx,fieldy) +" x + "+ CalculateAzero(fieldx,fieldy),Label.LEFT));
	...
	public static double CalculateLineX(double[] x,double[] y,double ValoreX){
		double ValoreRetta = (CalculateAone(x,y)*ValoreX)+CalculateAzero(x,y);
		return ValoreRetta;
	}  
}
